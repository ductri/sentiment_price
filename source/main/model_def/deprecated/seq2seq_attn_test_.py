...
from model_def.deprecated.seq2seq_attn import Seq2SeqAttn
...
class TestSeq2SeqAttn(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        inputs_idx = torch.randint(src_vocab_size, size=(batch_size, 10))
        model = Seq2SeqAttn(src_vocab_size=src_vocab_size, tgt_vocab_size=5, start_idx=1, end_idx=2)
        output = model(inputs_idx)
        self.assertEqual(output.size(), (batch_size, 100))
