...
class DecoderGreedyWithSrcInfer(nn.Module):
    ...
    def forward(self, enc_h_n, enc_c_n, enc_outputs, enc_inputs, starts_idx):
        """
...
class RawDecoder(nn.Module):
    def __init__(self, vocab_size):
        ...
        self.lstm = nn.LSTM(input_size=__embedding_size, hidden_size=__lstm_size,
                            num_layers=__lstm_num_layer,
                            bidirectional=False, dropout=__dropout_rate)
        self.output_mapping = nn.Linear(__lstm_size, vocab_size)
...
class AttnRawDecoder(nn.Module):
    def __init__(self, vocab_size, enc_output_size):
        ...
        pytorch_utils.register_buffer(self, 'lstm_num_layer', 3)
        pytorch_utils.register_buffer(self, 'dropout_rate', 0.3)
        pytorch_utils.register_buffer(self, 'half_window_size', 50)
        ...
        self.lstm = nn.LSTM(input_size=__embedding_size, hidden_size=__lstm_size,
                            num_layers=__lstm_num_layer,
                            bidirectional=False, dropout=__dropout_rate)
        self.attention = Attention(enc_output_size=enc_output_size, dec_output_size=__lstm_size)
        self.output_mapping = nn.Linear(__lstm_size+enc_output_size, vocab_size)
        self.dropout = nn.Dropout(p=__dropout_rate)
    ...
    def forward(self, inputs_idx, h_n_c_n, enc_outputs, step, *args):
        ...
        """
        __half_window_size = self.half_window_size.item()
        ...
        if step is None:
            ...
            for step in range(inputs_idx.size(0)):
                ...
                output_ = output_[0]
                local_enc_outputs = enc_outputs[max(0, step - __half_window_size):step + __half_window_size]
...
class AttnRawDecoderWithSrc(nn.Module):
    def __init__(self, vocab_size, enc_output_size, enc_embedding_size):
        ...
        self.dec_embedding = nn.Embedding(num_embeddings=vocab_size, embedding_dim=__embedding_size)
        self.lstm = nn.LSTM(input_size=__embedding_size+enc_embedding_size, hidden_size=__lstm_size,
                            num_layers=__lstm_num_layer,
                            bidirectional=False, dropout=__dropout_rate)
        self.attention = Attention(enc_output_size=enc_output_size, dec_output_size=__lstm_size)
        __shrink_size = 512
        self.shrink_mapping = nn.Linear(__lstm_size + enc_output_size, __shrink_size)
